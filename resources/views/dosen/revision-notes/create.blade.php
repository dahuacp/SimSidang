        ...
        <form method="POST" action="{{ route('dosen.revision-notes.store', $submission) }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="catatan_revisi" name="catatan_revisi" rows="5" required
                          ...
                          class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800">{{ old('catatan_revisi', $defaultRevision ?? '') }}</textarea>
            </div>
            <button type="submit"
