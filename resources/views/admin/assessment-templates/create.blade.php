...
@section('content')
    <h1 class="mb-4 text-xl font-bold text-gray-800 dark:text-white/90 sm:text-2xl">Tambah Template Penilaian</h1>

    <form method="POST" action="{{ route('admin.assessment-templates.store') }}" class="max-w-4xl">
        @csrf
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <select name="tipe_penilai" required
                    ...
                    <option value="dospem" @selected(old('tipe_penilai') == 'dospem')>Dosen Pembimbing</option>
                    <option value="penguji" @selected(old('tipe_penilai') == 'penguji')>Dosen Penguji</option>
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" min="1" name="nilai_penyebut" value="{{ old('nilai_penyebut', 15) }}" required
                           class="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800">
                ...
                <div>
                    ...
                    <input type="number" min="0" name="nilai_pengali" value="{{ old('nilai_pengali', 100) }}" required
                           class="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800">
        ...
        <div class="mt-6">
            @include('admin.assessment-templates._items')
        </div>
