...
@section('content')
    <h1 class="mb-4 text-xl font-bold text-gray-800 dark:text-white/90 sm:text-2xl">Tambah Program Studi</h1>

    <form method="POST" action="{{ route('admin.prodis.store') }}" class="max-w-3xl">
        @csrf
            ...
            <div class="md:col-span-2">
                ...
                <select name="fakultas_id" required
                    ...
                    @foreach($fakultas as $f)
                        <option value="{{ $f->id }}" @selected(old('fakultas_id') == $f->id)>{{ $f->nama_fakultas }} ({{ $f->kode_fakultas }})</option>
                    @endforeach
                </select>
