...
@section('title', 'Tambah Jenis Sidang')

@section('content')
    <h1 class="mb-4 text-xl font-bold text-gray-800 dark:text-white/90 sm:text-2xl">Tambah Jenis Sidang</h1>

    <form method="POST" action="{{ route('admin.jenis-sidangs.store') }}" class="max-w-3xl">
        @csrf
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Nama Jenis Sidang</label>
                <input type="text" name="nama" value="{{ old('nama') }}" placeholder="Mis. TA, KP, Milestone Design" required
