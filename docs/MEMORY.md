...
- **Tahap MVP aktif:** Tahap 1 (MVP inti) SELESAI + Tahap 2 (Peningkatan Operasional) SELESAI + Tahap 3 FR-05 SELESAI. ROADMAP.md: semua item Tahap 1, 2 & FR-05 Tahap 3 dicentang.
- **Fitur terakhir dikerjakan:** Tahap 3 — FR-05 Asisten Virtual Admin. Chat panel berbasis LLM OpenAI-compatible (provider abstraction via `LlmProviderInterface`). 4 tool read-only (getStudentProgress, getDosenWorkload, getStalledRevisions, getScheduleSummary). Guardrail: tool whitelist backend, read-only system prompt, rate limiting (10 req/min configurable), 50 message hard limit per conversation, `tool_calls` audit JSON column. Migration `assistant_conversations` + `assistant_messages`, model, service layer, controller, FormRequest, Alpine.js chat component, Blade view.
- **Blocker/isu terbuka:** (1) Warning build Vite: font `bootstrap-icons.woff/woff2` tidak ter-resolve (ikon font mungkin tidak tampil di semua browser — belum diverifikasi manual). (2) Git belum di-init ulang (`.git` dihapus saat scaffold). (3) `laravel boost:mcp` belum dikonfigurasi/test (route `_boost/*` terdaftar).
- **Environment:** Laravel 13.8.0, PHP 8.4.23, MariaDB 11.8.6 (MySQL-compatible), DB `sidangapp2`/user `sidang`/pass `sidang` @ 127.0.0.1:3306. `APP_NAME=SISIDANG`, `APP_LOCALE=id`, `FILESYSTEM_DISK=local`. Packages: `maatwebsite/excel` ^3.1, `barryvdh/laravel-dompdf` ^3.1 (dompdf 3.x). Tidak ada paket LLM tambahan — pakai `Illuminate\Support\Facades\Http` langsung.
...
- **Test:** 56 test Pest (SQLite :memory: + RefreshDatabase), **semua lulus** (143 assertion). `pint`, `npm run lint`, `npm run build` semua bersih/sukses.
